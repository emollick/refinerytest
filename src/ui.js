...
export class UIController {
  constructor(simulation) {
    ...
    this.marketElements = {
      headline: document.getElementById("market-headline"),
      futures: {
        gasoline: document.getElementById("market-futures-gasoline"),
        diesel: document.getElementById("market-futures-diesel"),
        jet: document.getElementById("market-futures-jet"),
      },
      cost: {
        gasoline: document.getElementById("market-cost-gasoline"),
        diesel: document.getElementById("market-cost-diesel"),
        jet: document.getElementById("market-cost-jet"),
      },
      margin: {
        gasoline: document.getElementById("market-margin-gasoline"),
        diesel: document.getElementById("market-margin-diesel"),
        jet: document.getElementById("market-margin-jet"),
      },
    };
    ...
    this.perBarrelFormatter = new Intl.NumberFormat("en-US", {
      style: "currency",
      currency: "USD",
      maximumFractionDigits: 0,
    });
    this.marginFormatter = new Intl.NumberFormat("en-US", {
      style: "currency",
      currency: "USD",
      maximumFractionDigits: 0,
      signDisplay: "always",
  ...
  _renderMetrics(metrics) {
    ...
    this._renderMarket(metrics);
  }

  _renderMarket(metrics) {
    if (!this.marketElements) {
      return;
    }
    const { headline, futures, cost, margin } = this.marketElements;
    if (headline) {
      headline.textContent = metrics.marketHeadline || "Coastal futures steady.";
    }
    const futuresData = metrics.futures || {};
    const costData = metrics.productionCost || {};
    const marginData = metrics.productMargin || {};
    Object.keys(PRODUCT_LABELS).forEach((product) => {
      const futureEl = futures?.[product];
      if (futureEl) {
        const value = futuresData[product] ?? 0;
        futureEl.textContent = `${this.perBarrelFormatter.format(Math.round(value))} / bbl`;
      }
      const costEl = cost?.[product];
      if (costEl) {
        const value = costData[product] ?? 0;
        costEl.textContent = `${this.perBarrelFormatter.format(Math.round(value))} / bbl`;
      }
      const marginEl = margin?.[product];
      if (marginEl) {
        const value = marginData[product] ?? 0;
        marginEl.textContent = `${this.marginFormatter.format(Math.round(value))} / bbl`;
        marginEl.classList.remove("positive", "negative");
        if (value >= 1) {
          marginEl.classList.add("positive");
        } else if (value <= -1) {
