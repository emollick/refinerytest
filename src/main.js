import { RefinerySimulation } from "./simulation.js?v=4";
import { UIController } from "./ui.js?v=4";
...
class TileRenderer {
  constructor(container, simulationInstance, unitDefs, pipelineDefs) {
    ...
    this.palettes = [
      {
        pavement: "#c7b296",
        pavementShadow: "#927b5c",
        water: "#0f3f7b",
        waterHighlight: "#1f6fc6",
        shore: "#92734a",
        grass: "#68a54f",
        green: "#7cbc62",
        field: "#b88446",
        ...
        roadLine: "#f5e1a8",
        walkway: "#d4b48b",
        grid: "rgba(32, 38, 54, 0.42)",
        outline: "#1a1e28",
        pipeBase: "#7fc6ff",
        pipeGlow: "rgba(235,246,255,0.78)",
        labelBg: "rgba(10, 16, 28, 0.68)",
        shoreFoam: "rgba(247, 247, 236, 0.7)",
      },
      {
        pavement: "#b6c0ce",
        pavementShadow: "#8894a6",
        water: "#15345d",
        waterHighlight: "#265489",
        shore: "#7c6f52",
        grass: "#5f9760",
        green: "#6ba89a",
        field: "#9c723f",
        ...
        roadLine: "#f0deb3",
        walkway: "#c5a77c",
        grid: "rgba(24, 32, 48, 0.48)",
        outline: "#171b26",
        pipeBase: "#8acfff",
        pipeGlow: "rgba(220,240,255,0.82)",
        labelBg: "rgba(12, 18, 30, 0.68)",
        shoreFoam: "rgba(247, 247, 236, 0.65)",
      },
    ];
  ...
  render(deltaSeconds, { flows, logistics }) {
    ...
    if (this.decorDynamic?.ship) {
      const ship = this.decorDynamic.ship;
      const bob = Math.sin(this.time * 1.6) * 1.8;
      ship.group.setAttribute("transform", `${ship.baseTransform} translate(0 ${bob.toFixed(2)})`);
      if (ship.wake) {
        const shipments = logistics?.shipments || [];
        const active = shipments.find((shipment) => shipment.product === "gasoline") || shipments[0];
        let readiness = 0.2;
        if (active) {
          const dueIn = Math.max(active.dueIn ?? 0, 0);
          const window = Math.max(active.window ?? 1, 0.01);
          readiness = clamp(1 - dueIn / window, 0, 1);
        }
        ship.wake.setAttribute("opacity", (0.25 + readiness * 0.55).toFixed(2));
      }
    }
  ...
  _createDecorNodes() {
    ...
    for (const item of this.decor) {
      if (item.type === "parking") {
        ...
        const { x, y } = this._tileToScreen(item.x, item.y);
        const hullLength = this.tileWidth * (item.length ?? 2.1) * 0.5;
        const hullDepth = this.tileHeight * 1.1;
        const baseTransform = `translate(${x.toFixed(1)} ${y.toFixed(1)})`;
        const group = createSvgElement("g", {
          class: "decor ship",
          transform: baseTransform,
        });
        const wake = createSvgElement("path", {
          class: "decor ship-wake",
          d: `M ${(-hullLength * 0.9).toFixed(1)} ${(hullDepth * 0.85).toFixed(1)} C ${(-hullLength * 0.55).toFixed(1)} ${(hullDepth * 1.15).toFixed(1)}, ${(hullLength * 0.55).toFixed(1)} ${(hullDepth * 1.1).toFixed(1)}, ${(hullLength * 0.88).toFixed(1)} ${(hullDepth * 0.8).toFixed(1)}`,
        });
        const hull = createSvgElement("polygon", {
          class: "decor ship-hull",
          points: pointsToString([
            [0, -hullDepth * 0.12],
            [hullLength * 0.72, hullDepth * 0.34],
            [0, hullDepth * 0.88],
            [-hullLength * 0.78, hullDepth * 0.36],
          ]),
        });
        const deck = createSvgElement("polygon", {
          class: "decor ship-deck",
          points: pointsToString([
            [0, hullDepth * 0.04],
            [hullLength * 0.55, hullDepth * 0.34],
            [0, hullDepth * 0.7],
            [-hullLength * 0.6, hullDepth * 0.32],
          ]),
        });
        const bridge = createSvgElement("polygon", {
          class: "decor ship-bridge",
          points: pointsToString([
            [-hullLength * 0.1, hullDepth * 0.22],
            [hullLength * 0.14, hullDepth * 0.32],
            [0, hullDepth * 0.52],
            [-hullLength * 0.24, hullDepth * 0.38],
          ]),
        });
        const stack = createSvgElement("rect", {
          class: "decor ship-stack",
          x: (-hullLength * 0.05).toFixed(1),
          y: (hullDepth * 0.28).toFixed(1),
          width: (hullLength * 0.12).toFixed(1),
          height: (hullDepth * 0.22).toFixed(1),
          rx: (hullLength * 0.02).toFixed(1),
        });
  ...
  _applyPalette() {
    ...
    this.tileNodes.forEach((tile) => {
      ...
      if (tile.overlay.type === "walkway") {
        ...
        const foamBase = palette.shoreFoam || lightenColor(palette.waterHighlight || palette.water, 0.25);
        tile.overlay.foam.setAttribute("stroke", foamBase);
    ...
    if (this.decorDynamic?.ship) {
      const { hull, deck, bridge, stack, wake } = this.decorDynamic.ship;
      const hullTone = mixColor(palette.shore || palette.pavement, "#8c3528", 0.6);
      const deckTone = mixColor(palette.walkway || palette.pavement, "#f0d29a", 0.5);
      const bridgeTone = lightenColor(deckTone, 0.2);
      const stackTone = lightenColor(bridgeTone, 0.1);
      if (hull) {
        hull.setAttribute("fill", hullTone);
        hull.setAttribute("stroke", darkenColor(hullTone, 0.55));
      }
      if (deck) {
        deck.setAttribute("fill", deckTone);
        deck.setAttribute("stroke", darkenColor(deckTone, 0.45));
      }
      if (bridge) {
        bridge.setAttribute("fill", bridgeTone);
        bridge.setAttribute("stroke", darkenColor(bridgeTone, 0.5));
      }
      if (stack) {
        stack.setAttribute("fill", stackTone);
        stack.setAttribute("stroke", darkenColor(stackTone, 0.6));
      }
      if (wake) {
        const wakeTone = applyAlpha(palette.waterHighlight || palette.water, 0.4);
        wake.setAttribute("stroke", wakeTone);
      }
    }
    for (const unit of this.unitDefs) {
  ...
  _buildDecor() {
    return [
      ...
      { type: "ship", x: 14.6, y: 8.8, length: 2.4 },
...
if ("ResizeObserver" in window) {
  const resizeObserver = new ResizeObserver(() => renderer.resizeToContainer(sceneContainer));
  resizeObserver.observe(mapViewport);
}
window.addEventListener("resize", () => renderer.resizeToContainer(sceneContainer));
renderer.resizeToContainer(sceneContainer);
