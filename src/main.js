...
function hexWithAlpha(hex, alpha){ const r=parseInt(hex.slice(1,3),16), g=parseInt(hex.slice(3,5),16), b=parseInt(hex.slice(5,7),16); return `rgba(${r},${g},${b},${alpha})`; }
...
/* ───────────────────── topology index (built after sim) ────────────────── */
...
class CanvasRenderer {
  constructor(container, simulation, unitDefs, pipelineDefs) {
    ...
    // geometry
    this.tileW = 64; this.tileH = 32; this.cols = 16; this.rows = 12;
    ...
    this.originY = this.tileH;

    // canvas
    this.canvas = document.createElement("canvas");
    this.canvas.className = "map-canvas";
    Object.assign(this.canvas.style, { width:"100%", height:"100%", display:"block", cursor:"grab", background:"#0b1018", imageRendering:"pixelated" });
    this.canvas.setAttribute("aria-hidden", "true");
    this.ctx = this.canvas.getContext("2d", { alpha: true });
    container.appendChild(this.canvas);

    // camera/state
    this.camera = { zoom:1, min:0.65, max:2.8, ox:0, oy:0, homeZoom:1, homeOX:0, homeOY:0, user:false };
    this.time=0; this.gridVisible=true; this.flowVisible=true; this.roadsVisible=true;
    this.highlighted=new Set(); this.selectedUnitId=null; this.hoverUnitId=null;
    this.pointer={x:0,y:0,active:false}; this.pan=null;
    this.bgColor = "#0b1018"; // visible even if nothing else draws

    // palettes + textures
    this.palettes = [
      { pavement:"#c8b79c", water:"#1d3b63", waterHi:"#2e5a87", shore:"#365b74", grass:"#7fb368", green:"#6fa793", field:"#b0823c", fieldAlt:"#d3a754", road:"#3c465c", roadLine:"#f0ddb1", walkway:"#c4af8a", outline:"#1c1d20", grid:"rgba(32,40,54,0.4)", pipeBase:"#76aee8", pipeGlow:"rgba(240,252,255,0.75)", labelBg:"rgba(12,16,22,0.6)" },
      { pavement:"#b5becb", water:"#16314e", waterHi:"#254a72", shore:"#314d6b", grass:"#6b9463", green:"#6fa0a9", field:"#9d7134", fieldAlt:"#c48a3d", road:"#3a4357", roadLine:"#e7deb4", walkway:"#bca27f", outline:"#16171c", grid:"rgba(20,32,48,0.42)", pipeBase:"#8ec8ff", pipeGlow:"rgba(226,246,255,0.8)", labelBg:"rgba(10,12,18,0.65)" },
      { pavement:"#b59b6b", water:"#1d4aa6", waterHi:"#2f76d4", shore:"#355672", grass:"#78a35a", green:"#6ea294", field:"#b28338", fieldAlt:"#d4a44b", road:"#5f6265", roadLine:"#e4d29f", walkway:"#b9a17a", outline:"#2b2e34", grid:"rgba(18,24,34,0.45)", pipeBase:"#6ea0d8", pipeGlow:"rgba(240,252,255,0.70)", labelBg:"rgba(10,14,20,0.62)" }
    ];
    this.paletteIndex = 2;
    ...
    // dpi + layout
    this.deviceScaleX = 1; this.deviceScaleY = 1;
  ...
  resizeToContainer(container){
    ...
    if ((rect.height|0) < 80) container.style.minHeight = "480px";
    ...
    this.displayW = w; this.displayH = h; this.dpr = dpr;
    this.deviceScaleX = dpr; this.deviceScaleY = dpr;
  ...
  resetView(){ this.camera.user=false; const {ox,oy,zoom}=this._centeredAt(this.camera.homeZoom); this.camera.zoom=zoom; this.camera.ox=ox; this.camera.oy=oy; this._clampCamera(); }
  ...
  beginPan(sx, sy){ this.canvas.style.cursor="grabbing"; this.pan={sx,sy,ox:this.camera.ox,oy:this.camera.oy}; this.camera.user=true; }
  panTo(sx, sy){ if (!this.pan) return; this.camera.ox=this.pan.ox+(sx-this.pan.sx); this.camera.oy=this.pan.oy+(sy-this.pan.sy); this._clampCamera(); }
  endPan(){ this.pan=null; this.canvas.style.cursor="grab"; }
  isPanning(){ return !!this.pan; }

  nudgeCamera(dx, dy){ this.camera.ox+=dx; this.camera.oy+=dy; this.camera.user=true; this._clampCamera(); }
  zoomAt(sx, sy, deltaY){
    ...
    const wx = (sx - this.camera.ox) / this.camera.zoom;
    const wy = (sy - this.camera.oy) / this.camera.zoom;
    this.camera.zoom = next; this.camera.ox = sx - wx * next; this.camera.oy = sy - wy * next;
    this.camera.user = true; this._clampCamera();
  }
  focusOnUnit(id, { onlyIfVisible=true }={}){
    const unit = this.unitDefs.find(u => u.id===id); if (!unit) return;
    ...
    this._moveCameraTo(c.x, c.y); this.camera.user=true;
  }
  ...
  render(dt, { flows }){
    ...
    // Always reset, clear, and paint a visible background
    ...
    if (this.pointer.active) this._drawPointer(ctx);
    // leave transform as-is for next frame; we always reset at top
  }
    ...
    for (let y=0; y<this.rows; y++){
      for (let x=0; x<this.cols; x++){
        ...
        if (this.roadsVisible && type === "road"){
          ...
          ctx.fillStyle = pal.road; pathPolygon(ctx, poly); ctx.fill();
        ...
        if (this.roadsVisible && type === "walkway"){
          const poly = this._walkwayPoly(x,y); ctx.fillStyle = pal.walkway; pathPolygon(ctx, poly); ctx.fill();
        }
        if (type === "shore"){
  ...
  _drawGrid(ctx, pal){
    ...
    for (let y=0; y<this.rows; y++)
      for (let x=0; x<this.cols; x++)
        { pathPolygon(ctx, this._tileDiamond(x,y)); ctx.stroke(); }
  }
  ...
  _drawUnits(ctx, pal){
    ...
    for (const u of this.unitDefs){
      ...
      ctx.fillStyle = hexWithAlpha(darken(toHex(u.color), .55), .45); pathPolygon(ctx, shadow); ctx.fill();
      ctx.fillStyle = toHex(u.color); ctx.strokeStyle = darken(toHex(u.color), .4); ctx.lineWidth = 1.5; pathPolygon(ctx, body); ctx.fill(); ctx.stroke();
      ctx.fillStyle = lighten(toHex(u.accent), .25); ctx.strokeStyle = darken(toHex(u.accent), .38); pathPolygon(ctx, roof); ctx.fill(); ctx.stroke();
      ...
      if (isSel || isHover) {
        ...
        ctx.lineWidth = 3;
        pathPolygon(ctx, diamond(c.x, c.y + this.tileH*0.2, this.tileW*1.2, this.tileH*0.5)); ctx.stroke();
        ctx.restore();
      }
      ...
      const barW = 52, gx = c.x - barW/2, gy = c.y + this.tileH*0.5;
      ctx.fillStyle = pal.labelBg; roundRect(ctx, gx, gy, barW, 10, 3); ctx.fill();
      ...
      ctx.fillStyle = "#f1f5ff"; ctx.fillText(u.name, c.x, c.y + this.tileH*0.85);
    }
  }
  _drawPointer(ctx){
    const p = this._diamondAt(this.pointer.x, this.pointer.y);
    ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 1.6; ctx.globalAlpha = 0.85; pathPolygon(ctx, p); ctx.stroke(); ctx.globalAlpha = 1;
  }

  /* math & helpers */
  _tileToScreen(wx, wy){ return { x: this.originX + (wx - wy) * (this.tileW/2), y: this.originY + (wx + wy) * (this.tileH/2) }; }
  _tileDiamond(tx, ty){ const p = this._tileToScreen(tx, ty); return [[p.x,p.y],[p.x+this.tileW/2,p.y+this.tileH/2],[p.x,p.y+this.tileH],[p.x-this.tileW/2,p.y+this.tileH/2]]; }
  _diamondAt(wx, wy){ const p = this._tileToScreen(wx, wy); return [[p.x,p.y],[p.x+this.tileW/2,p.y+this.tileH/2],[p.x,p.y+this.tileH],[p.x-this.tileW/2,p.y+this.tileH/2]]; }
  _highlightPoly(points){ const [top, right, bottom, left] = points; const center=[(top[0]+right[0]+bottom[0]+left[0])/4,(top[1]+right[1]+bottom[1]+left[1])/4]; const L=(a,b,t)=>[mix(a[0],b[0],t),mix(a[1],b[1],t)]; return [top, L(top,right,.45), center, L(top,left,.45)]; }
  _shadowPoly(points){ const [, right, bottom, left] = points; const L=(a,b,t)=>[mix(a[0],b[0],t),mix(a[1],b[1],t)]; return [right, L(right,bottom,.55), bottom, L(bottom,left,.4), L(right,points[0],.25)]; }
  _roadPoly(tx, ty, dir){ const p=this._tileDiamond(tx,ty); const L=(a,b,t)=>[mix(a[0],b[0],t),mix(a[1],b[1],t)]; if(dir==="ns")return[ L(p[0],p[1],.4), L(p[0],p[3],.4), L(p[2],p[3],.4), L(p[2],p[1],.4) ]; if(dir==="ew")return[ L(p[3],p[0],.4), L(p[1],p[0],.4), L(p[1],p[2],.4), L(p[3],p[2],.4) ]; const a=[ L(p[3],p[0],.4), L(p[1],p[0],.4), L(p[1],p[2],.4), L(p[3],p[2],.4) ]; const b=[ L(p[0],p[1],.4), L(p[0],p[3],.4), L(p[2],p[3],.4), L(p[2],p[1],.4) ]; return a.concat(b); }
  _roadCenterLine(tx,ty,dir){ const p=this._tileDiamond(tx,ty); const L=(a,b,t)=>[mix(a[0],b[0],t),mix(a[1],b[1],t)]; if(dir==="ns")return[ L(p[0],p[2],.12), L(p[0],p[2],.88) ]; return[ L(p[3],p[1],.12), L(p[3],p[1],.88) ]; }
  _walkwayPoly(tx,ty){ const c=this._tileDiamond(tx,ty); const L=(a,b,t)=>[mix(a[0],b[0],t),mix(a[1],b[1],t)]; return[ L(c[0],c[1],.6), L(c[0],c[3],.6), L(c[2],c[3],.6), L(c[2],c[1],.6) ]; }
  _centeredAt(zoom){ const b=this._mapBounds(); const cx=(b.minX+b.maxX)/2, cy=(b.minY+b.maxY)/2; const ox=this.canvas.width/2 - cx*zoom; const oy=this.canvas.height/2 - cy*zoom; return { ox, oy, zoom }; }
  _moveCameraTo(wx, wy){ this.camera.ox=this.canvas.width/2 - wx*this.camera.zoom; this.camera.oy=this.canvas.height/2 - wy*this.camera.zoom; this._clampCamera(); }
  _fitCameraToView(){ const b=this._mapBounds(); const padX=160*this.dpr, padY=140*this.dpr; const availW=this.canvas.width-padX, availH=this.canvas.height-padY; const scaleX=availW/Math.max(1,b.width), scaleY=availH/Math.max(1,b.height); const target=clamp(Math.min(scaleX,scaleY),this.camera.min,this.camera.max); const {ox,oy}=this._centeredAt(target); this.camera.homeZoom=target; this.camera.zoom=target; this.camera.homeOX=ox; this.camera.homeOY=oy; this.camera.ox=ox; this.camera.oy=oy; }
  _clampCamera(){ const b=this._mapBounds(); const z=this.camera.zoom; const mapW=b.width*z, mapH=b.height*z; const margin=48*this.dpr; const extraX=Math.max((this.canvas.width-mapW)/2,0), extraY=Math.max((this.canvas.height-mapH)/2,0); const effX=Math.min(margin,extraX), effY=Math.min(margin,extraY); const minOX= effX - b.minX*z, maxOX= this.canvas.width - effX - b.maxX*z; const loX=Math.min(minOX,maxOX), hiX=Math.max(minOX,maxOX); const minOY= effY - b.minY*z, maxOY= this.canvas.height - effY - b.maxY*z; const loY=Math.min(minOY,maxOY), hiY=Math.max(minOY,maxOY); this.camera.ox=clamp(this.camera.ox,loX,hiX); this.camera.oy=clamp(this.camera.oy,loY,hiY); }
  _mapBounds(){ const corners=[ this._tileToScreen(0,0), this._tileToScreen(this.cols,0), this._tileToScreen(0,this.rows), this._tileToScreen(this.cols,this.rows) ]; const xs=corners.map(p=>p.x), ys=corners.map(p=>p.y); const minX=Math.min(...xs), maxX=Math.max(...xs), minY=Math.min(...ys), maxY=Math.max(...ys); return { minX, maxX, minY, maxY, width:maxX-minX, height:maxY-minY }; }
  _buildTiles(){ const T=Array.from({length:this.rows},()=>Array.from({length:this.cols},()=> "pavement")); for(let y=0;y<this.rows;y++){ for(let x=0;x<this.cols;x++){ if (x<2 && y<6) T[y][x]="water"; else if (x<2 && y<8) T[y][x]="shore"; else if (y>=this.rows-3 && x<4) T[y][x]="field"; else if (y>=this.rows-4 && x<5) T[y][x]="field"; else if (y<3 && x>=this.cols-3) T[y][x]="water"; else if (x===2 && y>=6 && y<this.rows-1) T[y][x]="grass"; else if (y===this.rows-1 && x>=4) T[y][x]="grass"; else if (y>=3 && y<=4 && x>=3 && x<=5) T[y][x]="green"; const cur=T[y][x]; if (y===5 && x>=2 && x<=this.cols-2 && cur!=="water" && cur!=="shore") T[y][x]=cur.startsWith("road")?"road-cross":"road-ew"; if (x===7 && y>=2 && y<=this.rows-2 && cur!=="water" && cur!=="shore") T[y][x]=T[y][x].startsWith("road")?"road-cross":"road-ns"; if ((x===6||x===8||(y===6&&(x===5||x===9))) && !["water","shore"].includes(T[y][x]) && !T[y][x].startsWith("road")) T[y][x]="walkway"; }} return T; }
  _buildDecor(){ return [ {type:"parking",x:12.5,y:1.8,width:2.5,height:1.8}, {type:"booth",x:5.2,y:10.4}, {type:"flare",x:9.5,y:2.1}, {type:"dock",x:14.2,y:9.6}, {type:"barn",x:1.8,y:10.2}, {type:"recording",x:6.8,y:0.6} ]; }
  _makeDitherTextures(){ const pal=this.palettes[this.paletteIndex]; const mk=(base,dot)=>{ const c=document.createElement("canvas"); c.width=2;c.height=2; const g=c.getContext("2d"); g.fillStyle=base; g.fillRect(0,0,2,2); g.fillStyle=dot; g.fillRect(0,0,1,1); g.fillRect(1,1,1,1); return this.ctx.createPattern(c,"repeat"); }; this.tex={ pavement:mk(pal.pavement,darken(pal.pavement,.22)), grass:mk(pal.grass,darken(pal.grass,.25)), field:mk(pal.field,lighten(pal.field,.18)), water:mk(pal.water,darken(pal.water,.25)), green:mk(pal.green,darken(pal.green,.2))}; }
  _textureFor(type,pal){ if(type==="pavement")return this.tex.pavement; if(type==="grass")return this.tex.grass; if(type==="field")return this.tex.field; if(type==="water")return this.tex.water; if(type==="green")return this.tex.green; if(type==="shore")return this.tex.pavement; if(type.startsWith("road")||type==="walkway")return pal.pavement; return pal.pavement; }
  _colorFor(type,pal){ if(type==="pavement")return pal.pavement; if(type==="grass")return pal.grass; if(type==="field")return pal.field; if(type==="water")return pal.water; if(type==="green")return pal.green; if(type==="shore")return pal.shore; if(type.startsWith("road"))return pal.road; if(type==="walkway")return pal.walkway; return pal.pavement; }
  _logSizeOnce(){ if (this._logged) return; this._logged=true; const r=this.container.getBoundingClientRect(); console.debug(`[renderer] container ${Math.round(r.width)}×${Math.round(r.height)} css; canvas ${this.canvas.width}×${this.canvas.height} device; dpr=${this.dpr}`); }
}

/* ───────────────────────────────── boot ───────────────────────────────── */
function boot(){
  // Visible container (fallback if missing)
  const mapViewport =
    document.getElementById("map-viewport") ||
    (() => { const el=document.createElement("div"); el.id="map-viewport"; Object.assign(el.style,{minHeight:"480px",width:"100%",background:"#0b1018"}); document.body.prepend(el); return el; })();
  ...
  const flowToggleButton = menuBar?.querySelector('[data-action="view-toggle-flow"]') || null;
  const pipeToggleButton = menuBar?.querySelector('[data-action="view-toggle-pipes"]') || null;
  const roadToggleButton = menuBar?.querySelector('[data-action="view-toggle-roads"]') || null;

  // Simulation + topology
  const simulation = new RefinerySimulation();
  let processTopology = simulation.getProcessTopology?.() || {};
  let unitConnectionIndex = buildUnitConnectionIndex(processTopology);

  // Renderer (now class defined → no TDZ)
  const renderer = new CanvasRenderer(mapViewport, simulation, unitConfigs, pipelineConfigs);
  const surface  = renderer.getSurface();

  // UI (soft-fail)
  let ui;
  try { ui = new UIController(simulation); }
  catch (err) {
    console.warn("[ui] UIController failed; running with no‑op UI", err);
    ui = { setModeBadge(){}, refreshControls(){}, setScenario(){}, setRunning(){}, selectUnit(){}, update(){}, elements:{}, onRunningChange:null, onReset:null };
  }
  if (typeof ui.setModeBadge === "function") ui.setModeBadge("AUTO");

  // app state
  const unitPulseEntries = new Map(); const unitModeLabels = new Map();
  let selectedUnitId = null; let activePreset = "auto"; let lastPulseRefresh = 0; let activeMenu = null;
  ...
  toolbarScenarioButtons.forEach(b => b.addEventListener("click", () => { const s=b.dataset.scenario; if (!s) return; simulation.applyScenario(s); ui.setScenario(s); updateScenarioButtons(s); refreshTopologyAndLegend(); }));

  const sliderInputs = document.querySelectorAll('#hud input[type="range"]');
  sliderInputs.forEach(input => input.addEventListener("input", () => { updatePresetButtons(null); activePreset=null; if (typeof ui.setModeBadge === "function") ui.setModeBadge("CUSTOM"); }));
  if (ui.elements?.scenario) ui.elements.scenario.addEventListener("change", e => { updateScenarioButtons(e.target.value); refreshTopologyAndLegend(); });
  ...
  setFlowVisibility(true);   // pipelines shown
  setRoadVisibility(true);   // roads shown
  populateScenarioMenu();
  populateUnitMenu();
  initializeUnitPulseList();
  renderPrototypeNotes();

  // draw one immediate frame so you see something even if RAF is throttled
  renderer.render(0, { flows: simulation.getFlows?.() || {} });
  ...
  /* ───────────────────────── helpers/menus ─────────────────────── */
  function refreshTopologyAndLegend(){
  ...
  function initializeMenus(){
    ...
    menuBar.addEventListener("click", (e) => {
      ...
      else if (scenario) { simulation.applyScenario(scenario); ui.setScenario?.(scenario); updateScenarioButtons(scenario); refreshTopologyAndLegend(); }
  ...
  function setGridVisibility(v){ renderer.setGridVisible(v); updateToggleButton(gridToggleButton, v, "Hide Grid Overlay", "Show Grid Overlay"); }
  function setFlowVisibility(v){ renderer.setFlowVisible(v); updateToggleButton(flowToggleButton || pipeToggleButton, v, "Hide Pipelines", "Show Pipelines"); }
  function setRoadVisibility(v){ renderer.setRoadsVisible(v); updateToggleButton(roadToggleButton, v, "Hide Roads/Walkways", "Show Roads/Walkways"); }
  ...
  function exportSnapshot(){
    const snapshot = simulation.createSnapshot?.(); if (!snapshot) return;
  ...
  function populateScenarioMenu(){
    ...
    simulation.getScenarioList?.().forEach(s => {
      const b = document.createElement("button"); b.type="button"; b.className="menu-entry"; b.dataset.scenario=s.key; b.textContent=s.name; b.title=s.description; scenarioMenu.appendChild(b);
    });
    updateScenarioMenuActive(simulation.activeScenarioKey);
  }
  function updateScenarioMenuActive(key){ if (!scenarioMenu) return; scenarioMenu.querySelectorAll(".menu-entry").forEach(e => e.classList.toggle("active", e.dataset.scenario===key)); }

  function populateUnitMenu(){
    if (!unitMenu) return;
    unitMenu.innerHTML = "";
    simulation.getUnits?.().forEach(u => {
      const b = document.createElement("button"); b.type="button"; b.className="menu-entry"; b.dataset.unit=u.id; b.textContent=u.name; unitMenu.appendChild(b);
    });
  ...
  function initializeUnitPulseList(){
    ...
    simulation.getUnits?.().forEach(u=>{
      ...
      unitPulseEntries.set(u.id, { item, mode, status, incidents, loadFill: loadMeter.fill, loadValue: loadMeter.value, integrityFill: integMeter.fill, integrityValue: integMeter.value });
    });
    refreshUnitPulse(0,true);
  }
  function createPulseMeter(label,type){ const wrapper=document.createElement("div"); wrapper.className="pulse-meter"; wrapper.dataset.type=type; const labelEl=document.createElement("span"); labelEl.className="pulse-meter-label"; labelEl.textContent=label; const track=document.createElement("span"); track.className="pulse-meter-track"; const fill=document.createElement("span"); fill.className="pulse-meter-fill"; track.appendChild(fill); const value=document.createElement("span"); value.className="pulse-meter-value"; wrapper.append(labelEl,track,value); return { wrapper, fill, value }; }
  ...
  function collectActiveAlerts(){
    ...
    return simulation.getUnits?.().filter(u=>!!u.alert).map(u=>{
      ...
      return { type:"unit", unitId:u.id, label:u.name, name:u.name, severity:d.severity||u.alert||"warning", summary:buildUnitAlertSummary(u), detail:buildUnitAlertDescription(u), guidance:d.guidance||"", recordedAt:d.recordedAt||"" };
    }) || [];
  }
  ...
  function rebuildProcessLegend(){
    ...
    const existing = mapStatusPanel.querySelector("#process-legend"); if (existing) existing.remove();
  ...
  function highlightPipelinesForUnit(id){ if (!id){ clearPipelineHighlight(); return; } const list = unitConnectionIndex.get(id) || []; renderer.setHighlightedPipelines(list); }
  ...
  // simple copy of your formatters
  function buildUnitAlertSummary(u){ const d=u.alertDetail||u.lastIncident; if (d?.summary) return d.summary; if (u.status==="offline"){ if (u.emergencyOffline) return "Emergency shutdown"; if (u.manualOffline) return "Manual standby"; return "Offline for repairs"; } if (typeof u.integrity==="number" && u.integrity<0.5) return `Integrity ${Math.round(u.integrity*100)}%`; if (u.alert==="danger") return "Critical fault"; if (u.alert==="warning") return "Process warning"; return "Stable"; }
  function buildUnitAlertDescription(u){ const d=u.alertDetail||u.lastIncident; if (d?.cause && d?.guidance) return `${d.cause}. ${d.guidance}`; if (d?.cause) return d.cause; if (d?.guidance) return d.guidance; if (u.alert==="danger") return "Immediate intervention required."; if (u.alert==="warning"){ if (typeof u.integrity==="number") return `Integrity at ${Math.round(u.integrity*100)}%. Adjust maintenance or throughput.`; return "Monitor unit conditions closely."; } return "Online"; }
  function formatUnitStatus(u){ if (u.status==="offline") return formatOfflineStatus(u); if (u.alert) return buildUnitAlertSummary(u); if (typeof u.utilization==="number") return `Online • ${Math.round(u.utilization*100)}% load`; return "Online"; }
  function formatOfflineStatus(u){ const m = Math.max(1, Math.ceil(u.downtime||0)); if (u.alert) return `${buildUnitAlertSummary(u)} (${m}m)`; return `Offline (${m}m)`; }
  function formatIncidentCount(n){ return n===1 ? "1 incident" : `${n} incidents`; }
  function formatModeLabel(k){ if (!k) return "Balanced"; if (unitModeLabels.has(k)) return unitModeLabels.get(k); return k.split(/\s|_/).map(s => s.charAt(0).toUpperCase()+s.slice(1)).join(" "); }
  function updateMenuToggle(r){ if (!menuToggle) return; menuToggle.textContent = r ? "Pause" : "Resume"; menuToggle.setAttribute("aria-pressed", r ? "false":"true"); }

  function renderPrototypeNotes(){ /* optional: add your notes panel here if you use it */ }
}
