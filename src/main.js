...
class CanvasRenderer {
  constructor(container, simulation, unitDefs, pipelineDefs) {
    ...
    // world extents (isometric diamond)
    ...
    this.camera = {
      zoom: 1, min: 0.65, max: 2.8,
      ox: 0, oy: 0,
      homeZoom: 1, homeOX: 0, homeOY: 0,
      user: false
    };
    ...
    this.flowVisible  = true;  // pipelines
    this.roadsVisible = true;  // roads/walkways
  ...
  resetView(){
    ...
    this.camera.zoom = zoom; this.camera.ox = ox; this.camera.oy = oy;
    this._clampCamera();
  }
  ...
  _drawPointer(ctx){
    ...
    ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 1.6; ctx.globalAlpha = 0.85;
...
function boot(){
  ...
  // Toggle buttons (optional presence)
  ...
  // --- Renderer (class is already defined above, so no TDZ issue) ---
  ...
  try {
    ...
    ui = { setModeBadge(){}, refreshControls(){}, setScenario(){}, setRunning(){},
  ...
  toolbarScenarioButtons.forEach(b => b.addEventListener("click", () => {
    ...
    simulation.applyScenario(s); ui.setScenario(s); updateScenarioButtons(s);
...
/* Run after DOM is ready so DOM refs & fallback container are safe */
