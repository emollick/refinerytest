...
export class RefinerySimulation {
  constructor() {
    ...
    this.metrics = {
      ...
      futures: { gasoline: 0, diesel: 0, jet: 0 },
      productionCost: { gasoline: 0, diesel: 0, jet: 0 },
      productMargin: { gasoline: 0, diesel: 0, jet: 0 },
      marketHeadline: "Futures steady.",
    };
  ...
  _initMarketState() {
    const baseFutures = { gasoline: 104, diesel: 96, jet: 128 };
    return {
      baseFutures,
      futures: { ...baseFutures },
      productionCost: {
      ...
      margins: {
        gasoline: baseFutures.gasoline - 92,
        diesel: baseFutures.diesel - 86,
        jet: baseFutures.jet - 102,
      },
      headline: "Coastal futures steady.",
    };
  }
  ...
  _syncMarketMetrics() {
    ...
    this.metrics.futures = { ...this.market.futures };
    this.metrics.productionCost = { ...this.market.productionCost };
    this.metrics.productMargin = { ...this.market.margins };
    this.metrics.marketHeadline = this.market.headline;
  }
  ...
  _updateLogistics(context) {
    ...
    const report = {
      ...
      penalty: 0,
      demandShortage: 0,
      shortageByProduct: { gasoline: 0, diesel: 0, jet: 0 },
    };
    ...
    if (demandShortages.length) {
      demandShortages.forEach(({ product, shortage }) => {
        ...
        if (report.shortageByProduct[product] !== undefined) {
          report.shortageByProduct[product] += shortage;
        }
  ...
  _calculateProductStrain(product) {
    const unitGroups = {
      gasoline: ["reformer", "fcc", "alkylation"],
      diesel: ["hydrocracker", "distillation", "fcc"],
      jet: ["hydrocracker", "distillation", "reformer"],
    };
    const units = unitGroups[product] || [];
    if (!units.length) {
      return 0;
    }
    let total = 0;
    units.forEach((id) => {
      const unit = this.unitMap?.[id];
      if (!unit) {
        return;
      }
      const integrity = clamp(unit.integrity ?? 1, 0, 1);
      const offline = unit.status !== "online" ? 0.7 : 0;
      const downtimeFactor = unit.downtime > 0 ? clamp(unit.downtime / 36, 0, 0.4) : 0;
      const alertFactor = unit.alert === "danger" ? 0.3 : unit.alert === "warning" ? 0.15 : 0;
      const throttle = unit.overrideThrottle ?? 1;
      const throttleFactor = throttle < 1 ? (1 - throttle) * 0.3 : 0;
      const integrityFactor = (1 - integrity) * 0.6;
      const strain = clamp(offline + downtimeFactor + alertFactor + throttleFactor + integrityFactor, 0, 1);
      total += strain;
    });
    return clamp(total / units.length, 0, 1);
  }
  ...
  _updateGlobalEconomy({
    ...
    const totalOutput = Math.max(
      0.1,
      (production?.gasoline || 0) + (production?.diesel || 0) + (production?.jet || 0)
    );
    const totalCost = Math.max(0, crudeExpense + operatingExpense + penalty);
    const crudeFallback = this.metrics.crudeCostPerBbl || this._resolveCrudeCostPerBarrel(scenario);
    const baseCostPerBbl = totalOutput > 0 ? totalCost / totalOutput : crudeFallback;

    const shipmentReliability = clamp(this.metrics.shipmentReliability ?? 1, 0, 1);
    const reliability = clamp(this.metrics.reliability ?? 1, 0, 1);
    const maintenance = clamp(this.params.maintenance ?? 0.65, 0, 1);
    const environment = clamp(this.params.environment ?? 0.35, 0, 1);
    const demandShortages = logistics?.shortageByProduct || {};
    const storageLevels = this.storage?.levels || {};
    const storageCapacity = this.storage?.capacity || {};
    const baseFutures = this.market.baseFutures || {};
    const futures = { ...(this.market.futures || baseFutures) };
    const productionCost = {};
    const margins = {};
    let leadProduct = "gasoline";
    let leadMargin = -Infinity;
    const scenarioBias = {
      gasoline: scenario?.gasolineBias || 0,
      diesel: scenario?.dieselBias || 0,
      jet: scenario?.jetBias || 0,
    };

    const shipmentPenalty = Math.max(0, 1 - shipmentReliability);
    const reliabilityPenalty = Math.max(0, 1 - reliability);
    const incidentSignal = incidents?.incidents ? Math.min(0.4, incidents.incidents * 0.05) : 0;

    Object.keys(PRODUCT_LABELS).forEach((product) => {
      const share = production?.[product] ? production[product] / totalOutput : 0;
      const strain = this._calculateProductStrain(product);
      const capacity = storageCapacity[product] || 0;
      const level = storageLevels[product] || 0;
      const storageRatio = capacity ? clamp(level / capacity, 0, 1.2) : 0.5;
      const shortage = demandShortages[product] || 0;
      const shortageSignal = shortage > 0 ? clamp(shortage / 120, 0, 0.45) : 0;
      const inventorySignal =
        storageRatio < 0.55
          ? (0.55 - storageRatio) * 0.6
          : -(Math.max(0, storageRatio - 0.9) * 0.5);
      const stressSignal = this.marketStress * 0.25;
      const base = baseFutures[product] || 96;
      const targetFutures =
        base *
          (1 +
            scenarioBias[product] * 0.5 +
            strain * 0.8 +
            shortageSignal +
            reliabilityPenalty * 0.45 +
            shipmentPenalty * 0.35 +
            stressSignal +
            incidentSignal) +
        inventorySignal * base +
        share * 8;
      futures[product] = Math.max(28, futures[product] + (targetFutures - futures[product]) * 0.18);

      const maintenancePenalty = Math.max(0, 0.55 - maintenance) * 0.35;
      const environmentalDrag = environment * 0.05;
      const processPremium =
        1 +
        strain * 0.8 +
        shipmentPenalty * 0.2 +
        reliabilityPenalty * 0.25 +
        maintenancePenalty +
        scenarioBias[product] * 0.15 +
        environmentalDrag;

      const productCost = baseCostPerBbl * processPremium;
      productionCost[product] = productCost;
      const margin = futures[product] - productCost;
      margins[product] = margin;
      if (margin > leadMargin) {
        leadMargin = margin;
        leadProduct = product;
      }
    });

    let headline = this.market.headline || "Coastal futures steady.";
    if (leadMargin < -3) {
      headline = "Margins underwater across the slate.";
    } else if (leadMargin < 4) {
      headline = "Traders see slim spreads; mind your costs.";
    } else {
      headline = `${this._formatProductLabel(leadProduct)} futures lead refinery spreads.`;
    }

    if ((logistics?.failed || 0) > 0) {
      headline = `Missed liftings tighten ${this._formatProductLabel(leadProduct)} supply.`;
    } else {
      const tightest = Object.entries(demandShortages)
        .filter(([, value]) => value > 0)
        .sort((a, b) => b[1] - a[1])[0];
      if (tightest) {
        headline = `${this._formatProductLabel(tightest[0])} demand outpaces supply.`;
      }
    }

    this.market.futures = futures;
    this.market.productionCost = productionCost;
    this.market.margins = margins;
    this.market.headline = headline;
    this._syncMarketMetrics();
  }
  ...
  loadSnapshot(snapshot) {
    ...
    if (snapshot.market && typeof snapshot.market === "object") {
      const baseMarket = this._initMarketState();
      this.market = {
        ...baseMarket,
        ...snapshot.market,
        baseFutures: {
          ...baseMarket.baseFutures,
          ...(snapshot.market.baseFutures || {}),
        },
        futures: {
          ...baseMarket.futures,
          ...(snapshot.market.futures || {}),
        },
        productionCost: {
          ...baseMarket.productionCost,
          ...(snapshot.market.productionCost || {}),
        },
        margins: {
          ...baseMarket.margins,
          ...(snapshot.market.margins || {}),
        },
        headline:
          typeof snapshot.market.headline === "string"
            ? snapshot.market.headline
            : baseMarket.headline,
      };
    } else {
      this.market = this._initMarketState();
